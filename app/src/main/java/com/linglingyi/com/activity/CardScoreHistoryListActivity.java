 ...
 * @date: 2019/4/3
 */
public class CardScoreHistoryListActivity extends BaseActivity {
    ...
    RecyclerView rvList;
    private ScoreHistoyListAdapter mHistoryAdapter;
    private List<CardScoreModel> mList = new ArrayList<>();
    ...
    public void initData() {
        tvTitle.setText("查询记录");
        isScore = getIntent().getBooleanExtra("isScore", false);
        rvList.setLayoutManager(new LinearLayoutManager(context));
        mHistoryAdapter = new ScoreHistoyListAdapter(mList);
        mHistoryAdapter.setScore(isScore);
        ...
        OkClient.getInstance().post(httpParams, new OkClient.EntityCallBack<BaseEntity>(context, BaseEntity.class) {
            ...
            public void onSuccess(Response<BaseEntity> response) {
                ...
                if ("00".equals(model.getStr39())) {
                    List<CardScoreModel> list = JSONArray.parseArray(model.getStr57(), CardScoreModel.class);
                    mList.addAll(list);
