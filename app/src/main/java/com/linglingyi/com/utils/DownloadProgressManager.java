...
import  com.linglingyi.com.utils.DownloaderDBHelper.DownloadLog;
import  com.linglingyi.com.utils.DownloaderDBHelper.Downloading;
...
public class DownloadProgressManager {
    ...
    public synchronized SparseIntArray getProgress(String downloadPath) {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM " + DownloaderDBHelper.Downloading.TABLE_NAME + " WHERE download_path=?",
                new String[]{downloadPath});
        if (cursor != null) {
            SparseIntArray array = new SparseIntArray();
            while (cursor.moveToNext()) {
                DownloaderDBHelper.Downloading log =
                        DownloaderDBHelper.Downloading.convert(cursor);
    ...
    public synchronized void updateProgress(ArrayList<DownloaderDBHelper.Downloading> logs) {
        if (logs != null) {
            SQLiteDatabase db = downloaderDBHelper.getWritableDatabase();

            for (DownloaderDBHelper.Downloading log : logs) {
                db.update(DownloaderDBHelper.Downloading.TABLE_NAME, DownloaderDBHelper.Downloading.toContentValues(log),
                        DownloaderDBHelper.Downloading.FIELD_ID + "=?", new String[]{Integer.toString(log.getId())});
            }
    ...
    public synchronized void saveProgress(String downloadPath, SparseIntArray logs) {
        ArrayList<DownloaderDBHelper.Downloading> array =
                sparseArray2ArrayList(downloadPath, logs);
        saveProgress(downloadPath, array);
    }

    private ArrayList<DownloaderDBHelper.Downloading> sparseArray2ArrayList(
            String downloadUrl, SparseIntArray logs) {
        if (logs != null) {
            ArrayList<DownloaderDBHelper.Downloading> downloadings = new ArrayList<>();
    ...
    public synchronized boolean existDownload(String downloadPath) {
        SQLiteDatabase db = downloaderDBHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + DownloaderDBHelper.DownloadLog.TABLE_NAME +
