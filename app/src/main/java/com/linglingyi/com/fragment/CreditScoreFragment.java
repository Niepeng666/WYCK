 ...
 * @date: 2019/4/29
 */
public class CreditScoreFragment extends BaseFragment {
    ...
    private String name, idCard, bankAccount;
    ...
    public static CreditScoreFragment newInstance() {
    ...
    public int initLayout() {
        return R.layout.frag_credit_score;
    }

    @Override
    public void initData(View v) {
        scoreCost = StorageCustomerInfo02Util.getInfo("scoreCost", context);
    ...
    @OnClick({R.id.tv_history, R.id.tv_xieYi, R.id.btn_submit})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.tv_history:
                startActivity(new Intent(context, CardScoreHistoryListActivity.class).putExtra("isScore", true));
                break;
            case R.id.tv_xieYi:
                startActivity(new Intent(context, WebViewActivity.class)
                        .putExtra("title", "协议")
                        .putExtra("url", Constant.card_score_url));

                break;
            case R.id.btn_submit:
                name = etName.getText().toString();
                idCard = etIdCard.getText().toString();
                bankAccount = etBankNumber.getText().toString();
                ...
                if (CheckOutMessage.isEmpty(context, "银行卡号", bankAccount)) {
                    return;
                }
    ...
    private void requestData() {
        loadingDialog.show();
        HttpParams httpParams = new HttpParams();
        httpParams.put("3", "690016");
        httpParams.put("40", getMerNo());
        httpParams.put("42", idCard);
        httpParams.put("43", bankAccount);
        httpParams.put("44", name);
        OkClient.getInstance().post(httpParams, new OkClient.EntityCallBack<BaseEntity>(context, BaseEntity.class) {
            @Override
            public void onError(Response<BaseEntity> response) {
                super.onError(response);
