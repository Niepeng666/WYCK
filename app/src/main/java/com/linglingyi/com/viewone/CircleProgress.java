...
import com.linglingyi.com.utils.MiscUtil;
...
public class CircleProgress extends View {
    ...
    public final boolean ANTI_ALIAS = true;

    public final int DEFAULT_SIZE = 150;
    public final int DEFAULT_START_ANGLE = 270;
    public final int DEFAULT_SWEEP_ANGLE = 360;

    public final int DEFAULT_ANIM_TIME = 1000;

    public final int DEFAULT_MAX_VALUE = 100;
    public final int DEFAULT_VALUE = 50;
    public final int DEFAULT_HINT_SIZE = 15;
    public final int DEFAULT_VALUE_SIZE = 15;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        eWidth=context.getResources().getDimensionPixelSize(R.dimen.dp_1);
        initAttrs(attrs);
        initPaint();
        setValue(mValue);
    }
    ...
    private void initPaint() {
        ...
        mValuePaint.setTypeface(Typeface.createFromAsset(mContext.getAssets(),"fonts/cervo-medium.otf"));
        ...
        ePaint = new Paint();
        ePaint.setAntiAlias(antiAlias);
        ePaint.setColor(mContext.getResources().getColor(R.color.gray_A6BCE8));
        ePaint.setStyle(Paint.Style.STROKE);
        ePaint.setStrokeWidth(mContext.getResources().getDimension(R.dimen.dp_1));
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //求最小值作为实际值
        int minSize = w - getPaddingLeft() - getPaddingRight() - 2 * (int) maxArcWidth;
        //减去圆弧的宽度，否则会造成部分圆弧绘制在外围
        eRadius= (w - getPaddingLeft() - getPaddingRight() - eWidth*2) / 2;
        mRadius = minSize / 2-mContext.getResources().getDimensionPixelSize(R.dimen.dp_15);
        //获取圆的相关参数
        mCenterPoint.x = mCenterPoint.y = w / 2;
        //绘制圆弧的边界
        ...
        eRectF.left = mCenterPoint.x - eRadius - eWidth/2;
        eRectF.top = mCenterPoint.y - eRadius - eWidth/2;
        eRectF.right = mCenterPoint.x + eRadius + eWidth/2;
        eRectF.bottom = mCenterPoint.y + eRadius + eWidth/2;
        //计算文字绘制时的 baseline
        //由于文字的baseline、descent、ascent等属性只与textSize和typeface有关，所以此时可以直接计算
        //若value、hint、unit由同一个画笔绘制或者需要动态设置文字的大小，则需要在每次更新后再次计算
        mValueOffset = h - getBaselineOffsetFromY(mValuePaint);
        mHintOffset = mCenterPoint.y / 2 + mContext.getResources().getDimensionPixelSize(R.dimen.dp_15);
        updateArcPaint();
    ...
    private void drawText(Canvas canvas) {
        ...
        canvas.drawText(CommonUtils.formatNum_(String.valueOf(mValue)), mCenterPoint.x, mValueOffset -
                mContext.getResources().getDimensionPixelSize(R.dimen.dp_5), mValuePaint);
    ...
    private void drawArc(Canvas canvas) {
        ...
        canvas.drawArc(eRectF, mStartAngle-1, mSweepAngle+2, false, ePaint);
    ...
    private void updateArcPaint() {
        // 设置渐变
        LinearGradient gradient = new LinearGradient(0, getHeight(), getWidth(), getHeight(), mGradientColors, null, Shader.TileMode.MIRROR);
