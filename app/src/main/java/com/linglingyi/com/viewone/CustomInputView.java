...
public class CustomInputView extends EditText {
    ...
    private void getAtt(AttributeSet attrs) {
        ...
        circleColor = typedArray.getColor(R.styleable.PayPsdInputView_circleColor2, circleColor);
    ...
    private void initPaint() {
        ...
        circlePaint.setTextSize(CommonUtils.sp2px(mContext, 14));
        bottomLinePaint = getPaint(2, Paint.Style.FILL, bottomLineColor);

        borderPaint = getPaint(5, Paint.Style.STROKE, borderColor);

        divideLinePaint = getPaint(divideLineWidth, Paint.Style.FILL, borderColor);
        focusedColor=ContextCompat.getColor(mContext,R.color.background);
    }
    ...
    private void drawPsdCircle(Canvas canvas) {
        for (int i = 0; i < textLength; i++) {
            canvas.drawCircle(startX + i * 2 * startX,
                    ...
                    radius,
                    circlePaint);
//            canvas.drawText(getPasswordString(i), startX + i * 2 * startX, startY, circlePaint);
        }
    }
     ...
     * @return
     */
    public String getPasswordString(int position) {
